...
func (s *Server) getSettings(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Pool.QueryRow(r.Context(), `
		SELECT restaurant_name, phone, address, service_charge_percent::float8, vat_percent::float8, guest_wifi_ssid, guest_wifi_password, updated_at
		FROM restaurant_settings WHERE id = 1`).Scan(
		&st.RestaurantName, &st.Phone, &st.Address, &st.ServiceChargePercent, &st.VATPercent, &st.GuestWiFiSSID, &st.GuestWiFiPassword, &st.UpdatedAt,
	)
...
func (s *Server) updateSettings(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = s.Pool.Exec(r.Context(), `
		INSERT INTO restaurant_settings (id, restaurant_name, phone, address, service_charge_percent, vat_percent, guest_wifi_ssid, guest_wifi_password, updated_at)
