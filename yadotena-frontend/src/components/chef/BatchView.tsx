...
import { Layers, Coffee, Utensils, AlertTriangle, ChevronRight } from "lucide-react";
...
export function BatchView({ orders }: BatchViewProps) {
  ...
  activeOrders.forEach((o) => {
    const dest = o.tableId ? `Table ${o.tableId.replace(/^t/i, "")}` : o.type || "Takeaway";
    o.items?.forEach((item) => {
      const key = item.name.toLowerCase();
      const existing = itemMap.get(key) || { name: item.name, totalQuantity: 0, tickets: [] };
      existing.totalQuantity += item.quantity;
      
      const addonNames = item.selectedAddons?.map(a => typeof a === "string" ? a : (a as any).name || a) || [];
          ...
          return (
            <div
              ...
              className={`rounded-2xl border transition-all duration-200 bg-zinc-950 overflow-hidden ${
                isExpanded ? "border-amber-500/60 shadow-lg shadow-amber-950/30" : "border-zinc-850 hover:border-zinc-700"
              }`}
            >
              {/* Main Summary Bar */}
              <div
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <ChevronRight className={`h-5 w-5 text-zinc-400 transition-transform ${isExpanded ? "rotate-90 text-amber-400" : ""}`} />
                </div>
              </div>
                    ...
                    {batch.tickets.map((t, idx) => (
                      <div key={idx} className="p-3 rounded-xl border border-zinc-800 bg-zinc-950/80 space-y-1 text-xs">
                        <div className="flex justify-between font-bold">
                          <span className="text-white">{t.destination} <span className="text-zinc-500">#{t.orderId.slice(-6).toUpperCase()}</span></span>
                        ...
                        {t.addons.length > 0 && (
                          <div className="text-zinc-300 font-medium">
                            {t.addons.map((a, i) => (
                              <span key={i} className="inline-block mr-2 text-amber-300/90">+ {a}</span>
                            ))}
                          </div>
                        )}
