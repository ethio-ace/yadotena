...
export function ManagerSidebar({
  ...
  const navGroups = [
    {
      group: "OVERVIEW",
      items: [
        { name: "Today's Command", href: "/dashboard/manager", icon: LayoutDashboard },
      ],
    },
    {
      group: "CATALOG",
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "bg-card border-r flex flex-col justify-between transition-all duration-300 z-40",
      ...
      >
        <div>
          ...
          {/* Navigation Items */}
          <nav className="p-3 space-y-6 overflow-y-auto max-h-[calc(100vh-8rem)] scrollbar-none">
            {navGroups.map((group) => (
        ...
        <div className="p-3 border-t bg-muted/20">
          {(!isCollapsed || isOpenMobile) ? (
            <div className="p-3 rounded-xl border bg-card text-xs">
              <span className="font-bold text-foreground block">Yadotena POS</span>
              <span className="text-[10px] text-muted-foreground font-medium">Café Operations System</span>
            </div>
          ) : (
            <div className="text-center text-[10px] font-black text-muted-foreground">
              v1.0
            </div>
          )}
        </div>
