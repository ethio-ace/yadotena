...
- Waiter workspace (`/dashboard/waiter`) uses a **sidebar** (`WaiterSidebar`) instead of a bottom nav: Sell / Tables / Orders / Notifications (pending-count badge). Mobile gets a horizontal nav strip. Clicking any table — from the Sell floor strip or the Tables grid — opens ONE shared `TableDetailView` (stepper + items + totals + inline `TableAddItemsPanel` + Settle/View), so the two old table flows are merged; selection state lives in the page (`activeTable`). Occupied table cards show the ticket id (`#936827`). `TableAddItemsPanel` (inline search + category chips + per-item addon sheet) appends via `api.orders.addItems` and returns to the floor. `CafeOrderBuilder` still handles new orders/shop sales and, when extending an open order, shows the table's current items in its desktop side rail. Categories/addons fetch is gated on `builderOpen` (cafe-order / shop-sale / tables / active table).
