...
export function PopularityReport({
  ...
  const { rows, bucketLabel } = useMemo(
    () => computePopularityTrend({ range, orders, menuItems, topN: 8 }),
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground font-medium mt-0.5">
            Units per product across the period · {bucketLabel} buckets
          </p>
        </div>
      </div>
          ...
          {rows.map((p, idx) => (
            ...
            >
              ...
              <div className="text-amber-500">
                <Sparkline values={p.series.map((s) => s.units)} max={maxUnits} />
              </div>
