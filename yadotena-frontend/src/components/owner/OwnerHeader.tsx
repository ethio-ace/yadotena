...
import { CalendarRange, Bell, LogOut, Menu } from "lucide-react";
...
export function OwnerHeader({
  user = { name: "Business Owner", role: "OWNER" },
  attentionCount = 0,
  rangeDisplay,
  onOpenAttention,
  onToggleSidebar,
  ...
  return (
    <header className="h-16 bg-card border-b px-4 md:px-6 flex items-center justify-between shadow-sm sticky top-0 z-30">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-black text-base tracking-tight text-foreground">
                YADOTENA <span className="text-amber-600 dark:text-amber-400 font-extrabold">BUSINESS</span>
              </h1>
      ...
      {/* Right Reporting Period, Attention & User Profile */}
      <div className="flex items-center gap-3">
        ...
        <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-muted/40 border rounded-xl text-xs font-bold text-muted-foreground">
          ...
          <span className="text-foreground">{rangeDisplay || "Today"}</span>
        </div>
        ...
        <button
          ...
          title="Items requiring owner attention"
          aria-label={
            attentionCount > 0
